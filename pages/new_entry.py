...
def render(well_name: str = "Well 1"):
    ...
    with st.expander(f"{section_label} — Design Check", expanded=True):
        ...
        if _emw_auto:
            ...
            sw_press = (rho_d_disp * tvd_sw_disp * mw_td_disp) if None not in (rho_d_disp, tvd_sw_disp, mw_td_disp) else None
            ...
            lines = []
            lines.append(f"Applied EMW = ((SW Density × Water Depth × MW at TD) + (MW at TD × 0.052 × (Bottom TVD − Water Depth))) / (Bottom TVD / 0.052)")
            lines.append(f"            = (({_v(rho_d_disp)} × {_v(tvd_sw_disp, 1)} × {_v(mw_td_disp)}) + ({_v(mw_td_disp)} × 0.052 × ({_v(shoe_tvd_val, 1)} − {_v(tvd_sw_disp, 1)}))) / ({_v(shoe_tvd_val, 1)} / 0.052)")
